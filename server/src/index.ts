...
function corsAllowedOrigins(): string[] {
  const defaults = [
    "http://localhost:5173",
    "https://mylittleskies.vercel.app",
    "https://mylittleskies.vercel.app",
  ];
...
<head>
  ...
  <title>My Little Skies World Dashboard</title>
  <style>
...
<body>
  <main>
    <header>
      <div>
        <h1>My Little Skies World Dashboard</h1>
...
async function bootstrap() {
  ...
  httpServer.listen(PORT, () => {
    console.log(`My Little Skies server running on http://localhost:${PORT}`);
  });
}
