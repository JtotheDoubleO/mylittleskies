...
docker run -d --name mylittleskies-db -e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=mylittleskies -p 5432:5432 postgres:16
...
Production URLs:
- **Client**: https://mylittleskies.vercel.app (Vercel)
- **Server API**: Railway (or any Docker host)
- **Database**: Neon (recommended) or Railway Postgres

### Database Setup (Neon - Recommended)

**1. Create a Neon project**
- Go to [neon.tech](https://neon.tech) and create a new project.
- Copy the PostgreSQL connection string from the dashboard.

**2. Set `DATABASE_URL` on Railway**

```bash
railway variables set DATABASE_URL="postgresql://user:password@ep-xxx-xxx.us-east-1.aws.neon.tech/neondb?sslmode=require"
```

> Make sure `sslmode=require` is included in the Neon connection string.
...
> Use `migrate deploy` for production. Only use `migrate dev` locally.
...
npm i -g @railway/cli
railway login
railway link    # select the mylittleskies-server project
```
...
```bash
npx vercel login
npx vercel link   # select the mylittleskies project
```
...
```
VITE_SERVER_URL=https://mylittleskies-api-production.up.railway.app
```
...
| `CLIENT_URL` | `https://mylittleskies.vercel.app` (for CORS; comma-separated for extras) |
